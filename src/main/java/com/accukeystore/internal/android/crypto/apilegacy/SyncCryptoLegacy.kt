...
package com.accukeystore.internal.android.crypto.apilegacy
...
constructor(private val context: Context) : SyncCrypto {
  ...
  override fun unlock_keystore() {
    try {
      val intent = Intent(UNLOCK_ACTION)
  ...
  override fun create_key_if_not_available(alias: String) {
    if (get(alias) == null) {
      try {
        ...
        val success = put(getBytes(alias), key.encoded)
        if (!success) {
  ...
  private fun state(): State {
    ...
    when (mError) {
      NO_ERROR -> return State.UNLOCKED
